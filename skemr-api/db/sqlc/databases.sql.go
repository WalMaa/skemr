...
const getDatabasesByProjectId = `-- name: GetDatabasesByProjectId :many
SELECT id, display_name, db_name, username, password, host, port, ssl_mode, database_type, project_id, last_synced_at, last_sync_error, failed_connection_attempts, created_at, updated_at
FROM databases
WHERE project_id = $1
`

func (q *Queries) GetDatabasesByProjectId(ctx context.Context, projectID uuid.UUID) ([]Database, error) {
	rows, err := q.db.Query(ctx, getDatabasesByProjectId, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
